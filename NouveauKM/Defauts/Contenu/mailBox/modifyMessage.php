		...
		<form method="POST" id="formulaire" name="formulaire" enctype="multipart/form-data" action="Defauts\Contenu\mailBox\modifyMail.php">
			...
			<table class="table table-condensed" style="width : 100%;">
				...
				<tr>
					...
					<td><textarea name="description" class="form-control"><?php echo $myMessage[0]['descriptionTmp']; ?></textarea></td>
				</tr>
				<tr>
					<td colspan='2'>
						<br/>
					...
					<?php
						if(isset($explodeSection[1])){
							for($i = 1; $i < sizeof($explodeSection); $i++){
								...
								echo '<tr><td colspan="2"><span class="alert alert-danger pull-left">Attention : champ exclusivement réservé au code.</span></br><textarea id="exemple'.$ex.'" class="form-control" name="exemple'.$ex.'" required>'.$codeFinal[0].'</textarea></td></tr>';
								$ex++;
				...
				?>
					<td class="tdModifFunction">Pièce jointe : </td>
						<label><td><input type="file" name="pj" id="piecejointe" value="<?php echo $uploaddir; ?>"/></label><label> <?php echo $link; ?> </label></td>
				</tr>
				<tr>
